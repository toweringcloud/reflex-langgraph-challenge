...
### 21. 멀티모달 오디오 인터랙션 (STT/TTS 연동)

- **배경**: 텍스트 입력의 번거로움을 줄이고, 시각적 결과물(웹툰)과 함께 청각적 피드백(음성 설명)을 제공하여 시청각을 모두 만족하는 몰입형 사용자 경험(UX)이 필요함.
- **해결 및 기술적 구현**:
  - **음성 인식 (STT)**: Streamlit의 `st.audio_input` 위젯과 `ElevenLabs`의 Scribe API를 연동하여, 사용자의 마이크 음성을 텍스트로 즉시 변환하고 검색 프롬프트로 활용하도록 파이프라인을 구축함.
  - **음성 합성 (TTS)**: 이슈 웹툰 생성이 완료되면 `ElevenLabs`의 Multilingual v2 모델을 호출하여 해당 이슈 타이틀을 MP3 오디오 바이트로 변환함. 렌더링 즉시 `st.audio(autoplay=True)`를 통해 브라우저에서 자동 재생되도록 구현함.
- **효과**: 사용자가 마이크를 통해 에이전트와 자연스럽게 소통하고 생성된 결과를 음성으로 피드백받는 등, 보이스 챗봇 형태의 진일보한 상호작용(Interaction)을 완성함.
