...
def get_menu_instructions(
    ...
    return f"""
        ...
        - 오직 고객이 메뉴와 관련 없는 주제(예: 예약, 결제)를 명확히 언급할 때만 `make_handoff(triage_agent)`를 사용하세요.
    """
...
def get_order_instructions(
    ...
    return f"""
        ...
        - 답변을 마친 후 바로 Triage로 넘기지 마세요. 고객이 "고마워요", "다른 건요?" 같은 추가 질문을 할 기회를 주세요.
        - 오직 고객이 주문와 관련 없는 주제(예: 메뉴, 예약)를 명확히 언급할 때만 `make_handoff(triage_agent)`를 사용하세요.
    """
...
def get_reservation_instructions(
    ...
    return f"""
        ...
        - 답변을 마친 후 바로 Triage로 넘기지 마세요. 고객이 "고마워요", "다른 건요?" 같은 추가 질문을 할 기회를 주세요.
        - 오직 고객이 예약과 관련 없는 주제(예: 메뉴, 주문)를 명확히 언급할 때만 `make_handoff(triage_agent)`를 사용하세요.
    """
