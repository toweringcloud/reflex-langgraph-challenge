...
menu_agent.handoffs = [
    make_handoff(triage_agent, "현재 에이전트의 전문 업무(메뉴 이슈대응)가 완전히 종결되었거나, 사용자가 완전히 새로운 주제를 꺼내어 더 이상 답변할 수 없을 때만 호출하세요. 상담을 마무리하는 중이라면 호출하지 말고 직접 끝인사를 하세요."),
]

order_agent.handoffs = [
    make_handoff(triage_agent, "현재 에이전트의 전문 업무(주문 이슈대응)가 완전히 종결되었거나, 사용자가 완전히 새로운 주제를 꺼내어 더 이상 답변할 수 없을 때만 호출하세요. 상담을 마무리하는 중이라면 호출하지 말고 직접 끝인사를 하세요."),
]

reservation_agent.handoffs = [
    make_handoff(triage_agent, "현재 에이전트의 전문 업무(예약 이슈대응)가 완전히 종결되었거나, 사용자가 완전히 새로운 주제를 꺼내어 더 이상 답변할 수 없을 때만 호출하세요. 상담을 마무리하는 중이라면 호출하지 말고 직접 끝인사를 하세요."),
]
